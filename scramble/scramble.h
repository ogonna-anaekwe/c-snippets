 ...
 * 3. Select a random char btw the first and last i.e. btw 0 and 2. If 0 is selected, then 0 + 1 gives 'r' in the original string. We place this value in index 1
 * of the new string. If 2, then 2 + 1 gives the second e, and we place this char in index 1 of the new string instead.
 * 4. Update the value of the selected char in the array to -1.
 * 5. Create a new array with the non-negative values (This ensures the next time we randomly select, we're only selecting a value that has yet to be selected).
